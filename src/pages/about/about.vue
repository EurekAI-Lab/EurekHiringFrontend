...
const changeShowData = async () => {
  ...
  // 待处理 // 已处理
  const aKeyWard = isBigTabOneActive.value ? 'result' : 'next_step'
  const aValue = isBigTabOneActive.value ? ['PASS', 'FAIL'] : ['INVITE', 'DISCARD']
...
const clickScrollTopFunc = (type, status) => {
  if (type == 'one') {
    ...
    isSmallTabOneActive.value = status == 'pull'
  } else {
    let aFlag = false
    let aFailFlag = false
    interviewShowData.value.forEach((item) => {
...
const scrollElement = (atype) => {
  ...
  // 计算目标位置
  const targetPosition = document.getElementById(aId).offsetTop + 158 // 距离顶部50px的位置
  setTimeout(function () {
