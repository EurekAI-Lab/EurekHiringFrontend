          ...
          <view class="relative">
            ...
            <view class="absolute top-53 left-2 w-83 h-7 text-xs">
              <view v-if="isBigTabOneActive" class="flex flex-row justify-around">
                ...
                <!--  -->
                <view @click="handleInterviewResult(item.interview_result.id, item.jobseeker.user_id, 'DISCARD')"
                  class="rounded flex justify-center h-8 items-center w-14 bg-#ffe2e2 text-#ea5c5e">
                  弃用
                </view>
                <view @click="handleInterviewResult(item.interview_result.id, item.jobseeker.user_id, 'INVITE')"
          ...
          success: (res) => {
            if (res.statusCode === 200) {
              ...
              try {
                ...
                appApi.callback("inviteInterview", JSON.stringify({
                  positionId: '',
