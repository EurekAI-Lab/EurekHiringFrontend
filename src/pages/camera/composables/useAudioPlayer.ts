...
export function useAudioPlayer(options: AudioPlayerOptions = {}) {
  ...
  const createAudioContext = (): UniApp.InnerAudioContext => {
    ...
    context.onError((error: any) => {
      ...
      const audioError = new Error(error.errMsg || ERROR_MESSAGES.AUDIO_LOAD_FAILED)
      showErrorToast(audioError.message)
  ...
  const play = async (src?: string): Promise<boolean> => {
    try {
      ...
      await new Promise(resolve => 
        setTimeout(resolve, TIME_CONSTANTS.AUDIO_PLAY_DELAY)
      )
  ...
  const setLoadTimeout = () => {
    ...
    loadTimeout.value = setTimeout(() => {
      if (isLoading.value) {
        ...
        showErrorToast(timeoutError.message)
